...
from visualization.converter import OpenDriveConverter
from visualization.lanelet import ConversionLanelet
from visualization.lanelet_network import ConversionLaneletNetwork
from visualization.utils import encode_road_section_lane_width_id
...
class Network:
    ...
    def load_opendrive(self, opendrive):
    ...
    def export_lanelet_network(
        self, filter_types: list = None  # type: ignore
        ...
        for parametric_lane in self._planes:
            ...
            lanelet.predecessor = cast(LinkIndex, self._link_index).get_predecessors(parametric_lane.id_)
            lanelet.successor = cast(LinkIndex, self._link_index).get_successors(parametric_lane.id_)
        ...
        for lanelet in lanelet_network.lanelets:
            cast(ConversionLanelet, lanelet).description = str(lanelet.lanelet_id)
            lanelet_network.remove_lanelet(str(lanelet.lanelet_id))
            lanelet.lanelet_id = convert_to_new_lanelet_id(
                str(lanelet.lanelet_id), new_lanelet_ids_assigned
            )
            lanelet.predecessor = [
            ...
            if lanelet.adj_left is not None:
                lanelet.adj_left = convert_to_new_lanelet_id(
                    str(lanelet.adj_left), new_lanelet_ids_assigned
                )
            if lanelet.adj_right is not None:
                lanelet.adj_right = convert_to_new_lanelet_id(
                    str(lanelet.adj_right), new_lanelet_ids_assigned
                )
            lanelet_network.add_lanelet(lanelet)
    ...
    def export_commonroad_scenario(
        self, dt: float = 0.1, filter_types=None
    ) -> Scenario:
...
class LinkIndex:
            ...
            for lane_section in road.lanes.lane_sections:
                for lane in lane_section.allLanes:
                    ...
                    # Try to get next road
                        ...
                        road.link.successor is not None and road.link.successor.elementType != "junction"
                    ):
                    ...
                    # Try to get previous road
                        ...
                        road.link.predecessor is not None and road.link.predecessor.elementType != "junction"
                    ):
    ...
    def remove(self, parametric_lane_id):
        ...
        # Delete all occurrences in successor lists
